...
impl FloatConversion for Pixels {
    type Float = f32;
...
impl FloatConversion for UPixels {
    type Float = f32;
...
#[cfg(feature = "winit")]
impl From<winit::dpi::PhysicalSize<u32>> for Size<UPixels> {
    ...
    T: VectorLike<Unit>,
{
    ...
    fn add(self, rhs: T) -> Self::Output {
        let (rx, ry) = rhs.into_vector_components();
        Self {
            x: self.x + rx,
            y: self.y + ry,
        }
...
#[cfg(feature = "wgpu")]
impl From<Point<UPixels>> for wgpu::Origin3d {
    fn from(value: Point<UPixels>) -> Self {
        Self {
            x: value.x.0,
            y: value.y.0,
            z: 0,
...
pub trait VectorLike<Unit>: Sized {
    fn from_vector_components(components: (Unit, Unit)) -> Self;
    fn into_vector_components(self) -> (Unit, Unit);

    fn into_vector<OtherVectorLike>(self) -> OtherVectorLike
    where
        OtherVectorLike: VectorLike<Unit>,
    {
        OtherVectorLike::from_vector_components(self.into_vector_components())
    }
}

impl<Unit> VectorLike<Unit> for Point<Unit> {
    fn from_vector_components(components: (Unit, Unit)) -> Self {
        Self {
            x: components.0,
            y: components.1,
...
impl<Unit> VectorLike<Unit> for Size<Unit> {
    fn from_vector_components(components: (Unit, Unit)) -> Self {
        Self {
            width: components.0,
            height: components.1,
...
impl<Unit> VectorLike<Unit> for (Unit, Unit) {
    fn from_vector_components(components: Self) -> Self {
        components
    }

    fn into_vector_components(self) -> Self {
        self
    }
}
